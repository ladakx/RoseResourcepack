...
public class Hosting {
    ...
    public void handleRequest() {
        // Переконаємося, що плагін активний перед запуском завдання
        ...
        this.task = Bukkit.getScheduler().runTaskAsynchronously(RoseRP.getInstance(), () -> {
            try {
                ...
                // Если сокет был закрыт, игнорируем это исключение
                if (!serverSocket.isClosed()) {
                    e.printStackTrace();
                }
            } catch (Exception e) {
                e.printStackTrace(); // Рекомендуется логировать исключения для дальнейшего анализа
            }
